/*
=================================================================================================
  SCRIPT DE MIGRACIÓN DEFINITIVO: Personal -> Apex (Versión Consolidada vFinal-3 - CORREGIDO)
  ...
  Descripción:  Este script unifica todas las tareas de migración en un solo archivo ejecutable.
                ...
                incluyendo la lógica para migrar el estado 'O5' y las sanciones desde la
                tabla 'tblSancion'.

  Corrección:   Se agregó la lógica para migrar los registros de la tabla `Personal.dbo.tblSancion`,
                que no estaban siendo considerados en la versión anterior.
...
-- =============================================================================================
-- PASO 0: CREAR Y EJECUTAR EL PROCEDIMIENTO DE LIMPIEZA
-- =============================================================================================

PRINT '--- PASO 0: Creando y ejecutando procedimiento de limpieza ---';
...
BEGIN TRY
    ...
    ;WITH PoliciasStg AS (SELECT p.id_policia, RIGHT('00000000' + CAST(p.num_cedula AS VARCHAR(8)),8) AS CI, LEFT(LTRIM(RTRIM(p.nom_policia)), 60) AS Nombre, p.alta AS FechaIngreso, CASE WHEN p.id_estado = 1 THEN 1 ELSE 0 END AS Activo, p.id_seccion, p.id_puesto, p.id_turno, p.id_semana, p.id_horario, CASE LTRIM(RTRIM(UPPER(p.genero))) WHEN 'M' THEN 1 WHEN 'F' THEN 2 ELSE 4 END AS GeneroId, p.nivelestudio, p.estadocivil, p.id_grado, p.fecha_registro, p.fecha_actualizado, p.fecha_nacimiento, LEFT(LTRIM(RTRIM(p.domicilio)), 250) AS Domicilio, LEFT(LTRIM(RTRIM(p.correo)), 255) AS Email, LEFT(LTRIM(RTRIM(p.telefono)), 50) AS Telefono, LTRIM(RTRIM(p.escalafon)) AS EscalafonTxt, LTRIM(RTRIM(p.funcion)) AS FuncionTxt, p.departamento AS Ciudad, p.seccional, p.credencial, CASE WHEN UPPER(LTRIM(RTRIM(ISNULL(p.estudia, 'NO')))) = 'SI' THEN 1 ELSE 0 END AS Estudia FROM Personal.dbo.tblPolicias p),
    PoliciasMapeadas AS (SELECT s.CI, s.Nombre, s.FechaIngreso, s.Activo, s.id_seccion AS SeccionId, s.id_puesto AS PuestoTrabajoId, s.id_turno AS TurnoId, s.id_semana AS SemanaId, s.id_horario AS HorarioId, s.GeneroId, ne.Id AS NivelEstudioId, ec.Id AS EstadoCivilId, mg.IdGradoDestino AS CargoId, s.fecha_registro AS CreatedAt, s.fecha_actualizado AS UpdatedAt, s.fecha_nacimiento AS FechaNacimiento, s.Domicilio, s.Email, s.Telefono, e.Id AS EscalafonId, f.Id AS FuncionId, s.Ciudad, s.seccional AS Seccional, s.credencial AS Credencial, s.Estudia FROM PoliciasStg s LEFT JOIN #MapeoGrado mg ON s.id_grado = mg.IdGradoOrigen LEFT JOIN dbo.NivelEstudio ne ON ne.Nombre = s.nivelestudio COLLATE Modern_Spanish_CI_AS LEFT JOIN dbo.EstadoCivil ec ON ec.Nombre = s.estadocivil COLLATE Modern_Spanish_CI_AS LEFT JOIN dbo.Escalafon e ON e.Nombre = s.EscalafonTxt COLLATE Modern_Spanish_CI_AS LEFT JOIN dbo.Funcion f ON f.Nombre = s.FuncionTxt COLLATE Modern_Spanish_CI_AS)
    MERGE dbo.Funcionario AS T USING PoliciasMapeadas AS S ON T.CI = S.CI COLLATE DATABASE_DEFAULT
    WHEN NOT MATCHED BY TARGET THEN INSERT (CI,Nombre,FechaIngreso,Activo,CreatedAt,UpdatedAt,SeccionId,PuestoTrabajoId,TurnoId,SemanaId,HorarioId,GeneroId,NivelEstudioId,EstadoCivilId,CargoId,FechaNacimiento,Domicilio,Email,Telefono,EscalafonId,FuncionId,TipoFuncionarioId,Ciudad,Seccional,Credencial,Estudia) VALUES (S.CI,S.Nombre,S.FechaIngreso,S.Activo,S.CreatedAt,S.UpdatedAt,S.SeccionId,S.PuestoTrabajoId,S.TurnoId,S.SemanaId,S.HorarioId,S.GeneroId,S.NivelEstudioId,S.EstadoCivilId,S.CargoId,S.FechaNacimiento,S.Domicilio,S.Email,S.Telefono,S.EscalafonId,S.FuncionId,1,S.Ciudad,S.Seccional,S.Credencial,S.Estudia);
    ...
    /*================================================================
      PASO 3: Migrar Datos Relacionados (Estados Transitorios, Licencias, Históricos)
    =================================================================*/
    ...
    /*================================================================
      PASO 5: Migración de Novedades, Notificaciones y Dotaciones
    =================================================================*/
    PRINT '--- PASO 5: Migrando Novedades, Notificaciones, Dotaciones... ---';
    ...
    /*================================================================
      PASO 6: Migrar Estados Legales y Sanciones Legadas
    =================================================================*/
    PRINT '--- PASO 6: Migrando Estados Legales... ---';
    
    -- PASO 6A: Migración desde las banderas en tblPolicias
    ;WITH Estados AS (
    ...
    -- ====================================================================
    PRINT '--- PASO 6B: Migrando sanciones desde la tabla legada tblSancion... ---';
    ...
    /*================================================================
      PASO 7: Migrar Documentos Adjuntos de Estados Transitorios
    =================================================================*/
    PRINT '--- PASO 7: Migrando documentos adjuntos... ---';
    ...
    /*================================================================
      PASO 8: Migrar Fotos de Funcionarios
    =================================================================*/
    PRINT '--- PASO 8: Migrando fotos de funcionarios... ---';
...
-- =============================================================================================
-- PASO 9: CREAR OBJETOS DE BASE DE DATOS (VISTAS Y PROCEDIMIENTOS)
-- =============================================================================================
PRINT '--- PASO 9: Creando vistas y procedimientos almacenados... ---';
GO
