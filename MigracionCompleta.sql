...
BEGIN TRY
    ...
    -- INICIO DE LA SECCIÓN CORREGIDA Y AÑADIDA
-- ====================================================================

-- 1. Asegurarse de que el catálogo de texto completo existe
IF NOT EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE name = 'ft_ApexCatalog')
BEGIN
    CREATE FULLTEXT CATALOG ft_ApexCatalog AS DEFAULT;
END
GO

-- 2. Crear Índice en la tabla de Novedades (usando la columna correcta 'Texto')
IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = OBJECT_ID('dbo.Novedad'))
BEGIN
    -- NOTA: El nombre de la PK se toma del script que proporcionaste.
    -- Si tuvieras un error aquí, es porque el nombre de la PK es diferente.
    CREATE FULLTEXT INDEX ON dbo.Novedad(Texto LANGUAGE 3082) -- 3082 es para Español
    KEY INDEX PK__Novedad__3214EC0794366BB6 ON ft_ApexCatalog
    WITH CHANGE_TRACKING AUTO;
END
GO

-- 3. Crear Índice en la tabla de Funcionarios
IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = OBJECT_ID('dbo.Funcionario'))
BEGIN
    -- NOTA: El nombre de la PK se toma del script que proporcionaste.
    CREATE FULLTEXT INDEX ON dbo.Funcionario(Nombre LANGUAGE 3082)
    KEY INDEX PK__Funciona__3214EC07EA007E95 ON ft_ApexCatalog
    WITH CHANGE_TRACKING AUTO;
END
GO

-- Habilitar la indexación
ALTER FULLTEXT INDEX ON dbo.Novedad ENABLE;
GO
ALTER FULLTEXT INDEX ON dbo.Funcionario ENABLE;
GO

PRINT '¡Índices de texto completo creados y habilitados correctamente!';
-- ====================================================================
-- FIN DE LA SECCIÓN CORREGIDA Y AÑADIDA
-- ====================================================================
