...
BEGIN TRY
    ...
    -- Cargar las secciones desde la base Personal en lugar de utilizar valores fijos.
    MERGE dbo.Seccion AS tgt
        USING (
            SELECT id_seccion, nom_seccion
            FROM Personal.dbo.tblSecciones
        ) AS src(Id,Nombre)
    ON tgt.Id = src.Id
    WHEN MATCHED AND tgt.Nombre <> src.Nombre COLLATE DATABASE_DEFAULT
        THEN UPDATE SET Nombre = src.Nombre
    WHEN NOT MATCHED
        THEN INSERT (Id,Nombre,CreatedAt) VALUES (src.Id,src.Nombre,GETDATE());
    SET IDENTITY_INSERT dbo.Seccion OFF;
    
    SET IDENTITY_INSERT dbo.Turno ON;
    -- Migrar los turnos dinámicamente desde Personal.dbo.tblTurnos
    MERGE dbo.Turno AS tgt
        USING (
            SELECT id_turno, nom_turno
            FROM Personal.dbo.tblTurnos
        ) AS src(Id,Nombre)
    ON tgt.Id = src.Id
    WHEN MATCHED AND tgt.Nombre <> src.Nombre COLLATE DATABASE_DEFAULT
        THEN UPDATE SET Nombre = src.Nombre
    ...
    -- Migrar las semanas dinámicamente desde Personal.dbo.tblSemanas
    MERGE dbo.Semana AS tgt
        USING (
            SELECT id_semana, nom_semana
            FROM Personal.dbo.tblSemanas
        ) AS src(Id,Nombre)
    ON tgt.Id = src.Id
    WHEN MATCHED AND tgt.Nombre <> src.Nombre COLLATE DATABASE_DEFAULT
        THEN UPDATE SET Nombre = src.Nombre
    ...
    -- Migrar los horarios dinámicamente desde Personal.dbo.tblHorarios
    MERGE dbo.Horario AS tgt
        USING (
            SELECT id_horario, nom_horario
            FROM Personal.dbo.tblHorarios
        ) AS src(Id,Nombre)
    ON tgt.Id = src.Id
    WHEN MATCHED AND tgt.Nombre <> src.Nombre COLLATE DATABASE_DEFAULT
        THEN UPDATE SET Nombre = src.Nombre
