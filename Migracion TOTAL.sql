/*
===============================================================================
  SCRIPT DE MIGRACIÓN COMPLETO: Personal -> Apex
  Autor:        Equipo de migración
  Fecha:        26/07/2025
  Descripción: Migra todos los datos desde la base 'Personal' hacia 'Apex'.
                Incluye limpieza preventiva, población de catálogos, datos
                dependientes y migración de fotos de funcionarios.

  Notas:
  - Este script está diseñado para ejecutarse en una nueva conexión hacia
  ...
  - [CORRECCIÓN]: Se ha añadido el PASO 4 para migrar las fotos.
===============================================================================
*/
...
BEGIN TRY
    /*
      ...
      IDENTITY_INSERT activo al iniciar la migración.
    */
    ...
    /*
      Iniciamos una transacción para asegurar que todas las etapas de la
      migración se ejecuten de forma atómica.
    */
    BEGIN TRANSACTION;
    ...
    USING (VALUES (1,'General'),(2,'Médica'),(3,'Especial')) AS src(Id,Nombre)
        ON tgt.Id = src.Id
    WHEN NOT MATCHED THEN
    ...
    USING (VALUES
        ...
        (3059,'DEPORTE Y RECREACIÓN'),(3060,'ÁREA DE TRATAMIENTO'),(3061,'COORDINACION OPERATIVA'),
        ...
        (5074,'COORDINADOR OPERATIVO'),(5075,'COORDINADOR TÉCNICO'),(5076,'COORDINADOR DE RECLUSIÓN'),
    ...
    USING (VALUES (1,'Soltero/a'),(2,'Casado/a'),(3,'Divorciado/a'),(4,'Viudo/a'),(5,'Unión libre'),(6,'Separado/a'),(7,'No especifica')) AS src(Id,Nombre)
      ON tgt.Id = src.Id
    WHEN NOT MATCHED THEN INSERT (Id,Nombre) VALUES(src.Id,src.Nombre)
    WHEN MATCHED AND tgt.Nombre <> src.Nombre THEN UPDATE SET Nombre = src.Nombre;
    ...
    USING (VALUES
        ...
        (5,'Secundaria completa'),(6,'Técnico terciario'),(7,'Universitario incompleto'),
    ...
    USING (VALUES (1,'Designación',0),(2,'Enfermedad',0),(3,'Sanción',0),(4,'Orden Cinco',0),(5,'Retén',0),(6,'Sumario',0)) AS src(Id,Nombre,EsJerarquico)
      ON tgt.Id = src.Id
    WHEN NOT MATCHED THEN
    ...
    INSERT INTO #MapeoNivelEstudio VALUES
        ('CICLO BÁSICO COMPLETO',5),('CICLO BÁSICO INCOMPLETO',4),('EDUCACIÓN PRIMARIA',3),
        ('EDUCACION SECUNDARIA',5),('EDUCACIÓN TERCIARIA',6),('PROFESIONAL',8),('',10);
    ...
    USING (
        SELECT l.id_licencia, mp.FuncionarioId, l.id_ausencia AS TipoLicenciaId,
               ...
               l.Cantidad_dentro_del_período, l.Unidad, l.Afecta_a_días, l.C_Presentó_certificado_,
               l.Usuario_aprobó_anuló_rechazó, l.Fecha_aprobación_anulación_rechazo, l.Comentario
    ...
    WHEN NOT MATCHED BY TARGET THEN
      INSERT (Id,FuncionarioId,TipoLicenciaId,inicio,finaliza,fecha_registro,fecha_actualizado,usuario,
              datos,estado,Unidad_Ejecutora,Unidad_Organizativa,Cantidad,Cantidad_dentro_del_período,
              Unidad,Afecta_a_días,C_Presentó_certificado_,Usuario_aprobó_anuló_rechazó,
              Fecha_aprobación_anulación_rechazo,Comentario)
      VALUES(SRC.id_licencia,SRC.FuncionarioId,SRC.TipoLicenciaId,SRC.inicio,SRC.finaliza,
             ...
             SRC.Unidad_Organizativa,SRC.Cantidad,SRC.Cantidad_dentro_del_período,SRC.Unidad,
             SRC.Afecta_a_días,SRC.C_Presentó_certificado_,SRC.Usuario_aprobó_anuló_rechazó,
             SRC.Fecha_aprobación_anulación_rechazo,SRC.Comentario);
    ...
    -- 3.4 Tablas históricas
    PRINT 'Paso 3.4: Migrando tablas históricas...';
    ...
    ;WITH NoctAgreg AS (
        SELECT hn.id_policia,hn.año,hn.mes,SUM(hn.minutos) AS minutos
          FROM Personal.dbo.tblHistoricoNocturnidad hn
         GROUP BY hn.id_policia,hn.año,hn.mes
    )
    INSERT INTO dbo.HistoricoNocturnidad (FuncionarioId,Anio,Mes,Minutos)
    SELECT m.FuncionarioId,na.año,na.mes,na.minutos
      FROM NoctAgreg na
      JOIN dbo.MapPoliciaFunc m ON m.id_policia = na.id_policia
     WHERE NOT EXISTS (
         SELECT 1 FROM dbo.HistoricoNocturnidad hn
          WHERE hn.FuncionarioId = m.FuncionarioId AND hn.Anio = na.año AND hn.Mes = na.mes
    );

    ;WITH PresAgreg AS (
        SELECT hp.id_policia,hp.año,hp.mes,SUM(hp.minutos) AS minutos,SUM(hp.dias) AS dias,
               MIN(hp.incidencia) AS incidencia,MIN(hp.observaciones) AS observaciones
          FROM Personal.dbo.tblHistoricoPresentismo hp
         GROUP BY hp.id_policia,hp.año,hp.mes
    )
    INSERT INTO dbo.HistoricoPresentismo (FuncionarioId,Anio,Mes,Minutos,Dias,Incidencia,Observaciones)
    SELECT m.FuncionarioId,pa.año,pa.mes,pa.minutos,pa.dias,pa.incidencia,pa.observaciones
      FROM PresAgreg pa
      JOIN dbo.MapPoliciaFunc m ON m.id_policia = pa.id_policia
     WHERE NOT EXISTS (
         SELECT 1 FROM dbo.HistoricoPresentismo hp
          WHERE hp.FuncionarioId = m.FuncionarioId AND hp.Anio = pa.año AND hp.Mes = pa.mes
    );

    ;WITH ViatAgreg AS (
        SELECT hv.id_policia,hv.año,hv.mes,MIN(hv.incidencia) AS incidencia,MIN(hv.motivo) AS motivo
          FROM Personal.dbo.tblHistoricoViaticos hv
         GROUP BY hv.id_policia,hv.año,hv.mes
    )
    INSERT INTO dbo.HistoricoViatico (FuncionarioId,Anio,Mes,Incidencia,Motivo)
    SELECT m.FuncionarioId,va.año,va.mes,va.incidencia,va.motivo
      FROM ViatAgreg va
      JOIN dbo.MapPoliciaFunc m ON m.id_policia = va.id_policia
     WHERE NOT EXISTS (
         SELECT 1 FROM dbo.HistoricoViatico hv
          WHERE hv.FuncionarioId = m.FuncionarioId AND hv.Anio = va.año AND hv.Mes = va.mes
    );
    PRINT 'Paso 3.4: Tablas históricas migradas.';
    ...
    /*================================================================
      ...
      Se migran todas las fotos del historial y se asigna la más
