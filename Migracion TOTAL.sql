/*
===============================================================================
  SCRIPT DE MIGRACIÓN COMPLETO: Personal -> Apex (CORREGIDO v2)
  Autor:        Equipo de migración (revisado por Gemini)
  Fecha:        01/08/2025
  Descripción: Migra todos los datos desde la base 'Personal' hacia 'Apex'.
                Este script ha sido corregido para resolver conflictos de
                intercalación (collation) entre las bases de datos.
...
BEGIN TRY
    ...
    MERGE dbo.TipoLicencia AS tgt USING (SELECT id_ausencia, nom_ausencia, CASE WHEN ausente = 'SI' THEN 1 ELSE 0 END, CASE WHEN viatico = 'NO' THEN 1 ELSE 0 END, CASE WHEN presentismo = 'NO' THEN 1 ELSE 0 END, CASE WHEN habil = 'SI' THEN 1 ELSE 0 END FROM Personal.dbo.tblAusencias) AS src(Id,Nombre,EsAusencia,SuspendeViatico,AfectaPresentismo,EsHabil) ON tgt.Id = src.Id WHEN NOT MATCHED THEN INSERT (Id,Nombre,EsAusencia,SuspendeViatico,AfectaPresentismo,EsHabil,CreatedAt,CategoriaAusenciaId) VALUES(src.Id,src.Nombre,src.EsAusencia,src.SuspendeViatico,src.AfectaPresentismo,src.EsHabil,GETDATE(),1);
    ...
    INSERT INTO #MapeoEstado(IdOrigen, IdDestino) SELECT p.id_estado, a.Id FROM Personal.dbo.tblEstados p JOIN dbo.Estado a ON p.nom_estado = a.Nombre;
    
    ;WITH PoliciasStg AS (
        SELECT p.id_policia AS IdPolicia, RIGHT('00000000' + CAST(p.num_cedula AS VARCHAR(8)),8) AS CI, LTRIM(RTRIM(p.nom_policia)) AS Nombre, p.alta AS FechaIngreso,
               ...
               p.fecha_registro AS CreatedAt, p.fecha_actualizado AS UpdatedAt, p.fecha_nacimiento AS FechaNacimiento, LTRIM(RTRIM(p.domicilio)) AS Domicilio,
               LTRIM(RTRIM(p.correo)) AS Email, LTRIM(RTRIM(p.telefono))  AS Telefono, LTRIM(RTRIM(p.escalafon)) AS EscalafonTxt, LTRIM(RTRIM(p.funcion)) AS FuncionTxt
    ...
    PoliciasMapeadas AS (
        SELECT s.CI, s.Nombre, s.FechaIngreso, s.Activo, meE.IdDestino AS EstadoId, s.SeccionId, s.PuestoTrabajoId, s.TurnoId, s.SemanaId, s.HorarioId,
               ...
               s.CargoId, s.CreatedAt, s.UpdatedAt, s.FechaNacimiento, s.Domicilio, s.Email,s.telefono, e.Id AS EscalafonId, f.Id AS FuncionId
        FROM PoliciasStg s
    ...
    WHEN MATCHED THEN
      UPDATE SET tgt.Nombre = src.Nombre, tgt.FechaIngreso = src.FechaIngreso, tgt.Activo = src.Activo, tgt.EstadoId = src.EstadoId, tgt.SeccionId = src.SeccionId,
                 tgt.PuestoTrabajoId = src.PuestoTrabajoId, tgt.TurnoId = src.TurnoId, tgt.SemanaId = src.SemanaId, tgt.HorarioId = src.HorarioId,
                 tgt.GeneroId = src.GeneroId, tgt.NivelEstudioId = src.NivelEstudioId, tgt.EstadoCivilId = src.EstadoCivilId, tgt.CargoId = src.CargoId,
                 tgt.FechaNacimiento = src.FechaNacimiento, tgt.Domicilio = src.Domicilio, tgt.Email = src.Email,tgt.Telefono = src.Telefono, tgt.EscalafonId = src.EscalafonId,
                 tgt.FuncionId = src.FuncionId, tgt.UpdatedAt = SYSUTCDATETIME()
    ...
    /*================================================================
      PASO 3: Migrar tablas dependientes (el resto del script se mantiene igual)
    =================================================================*/
    PRINT 'Paso 3: Migrando tablas dependientes...';
    
    SET IDENTITY_INSERT dbo.EstadoTransitorio ON;
    ;WITH Detalles AS ( SELECT u.id_estado,u.id_policia,u.categoria AS categoria, d.descripcion,d.fecha_inicio,d.fecha_fin FROM Personal.dbo.tblEstadosUnificados u LEFT JOIN Personal.dbo.tblDesignacionesDetalle d ON d.id_estado = u.id_estado AND u.categoria = 'D' UNION ALL SELECT u.id_estado,u.id_policia,u.categoria, e.descripcion,e.fecha_inicio,e.fecha_fin FROM Personal.dbo.tblEstadosUnificados u LEFT JOIN Personal.dbo.tblEnfermedadDetalle e ON e.id_estado = u.id_estado AND u.categoria = 'E' UNION ALL SELECT u.id_estado,u.id_policia,u.categoria, s.descripcion,s.fecha_inicio,s.fecha_fin FROM Personal.dbo.tblEstadosUnificados u LEFT JOIN Personal.dbo.tblSancionesDetalle s ON s.id_estado = u.id_estado AND u.categoria = 'N' UNION ALL SELECT u.id_estado,u.id_policia,u.categoria, o.descripcion,o.fecha_inicio,o.fecha_fin FROM Personal.dbo.tblEstadosUnificados u LEFT JOIN Personal.dbo.tblOrdenCincoDetalle o ON o.id_estado = u.id_estado AND u.categoria = 'O' UNION ALL SELECT u.id_estado,u.id_policia,u.categoria, r.descripcion,r.fecha_reten AS fecha_inicio,NULL AS fecha_fin FROM Personal.dbo.tblEstadosUnificados u LEFT JOIN Personal.dbo.tblRetenesDetalle r ON r.id_estado = u.id_estado AND u.categoria = 'R' UNION ALL SELECT u.id_estado,u.id_policia,u.categoria, su.descripcion,su.fecha_inicio,su.fecha_fin FROM Personal.dbo.tblEstadosUnificados u LEFT JOIN Personal.dbo.tblSumariosDetalle su ON su.id_estado = u.id_estado AND u.categoria = 'S' ), Prim AS ( SELECT d.*, ROW_NUMBER() OVER (PARTITION BY d.id_estado ORDER BY d.fecha_inicio) AS rn FROM Detalles d )
    MERGE dbo.EstadoTransitorio AS tgt
    USING ( SELECT p.id_estado AS Id, mp.FuncionarioId, CASE p.categoria WHEN 'D' THEN 1 WHEN 'E' THEN 2 WHEN 'N' THEN 3 WHEN 'O' THEN 4 WHEN 'R' THEN 5 WHEN 'S' THEN 6 END AS TipoEstadoTransitorioId, CAST(COALESCE(p.fecha_inicio,SYSUTCDATETIME()) AS date) AS FechaDesde, p.fecha_fin AS FechaHasta, p.descripcion AS Observaciones FROM Prim p JOIN dbo.MapPoliciaFunc mp ON mp.id_policia = p.id_policia WHERE p.rn = 1 ) AS src
      ON tgt.Id = src.Id
    WHEN NOT MATCHED THEN INSERT (Id,FuncionarioId,TipoEstadoTransitorioId,FechaDesde,FechaHasta,Observaciones,CreatedAt) VALUES(src.Id,src.FuncionarioId,src.TipoEstadoTransitorioId,src.FechaDesde,src.FechaHasta,src.Observaciones,SYSUTCDATETIME());
