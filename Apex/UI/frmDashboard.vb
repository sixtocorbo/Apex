...
#Region "Lógica de Navegación Principal"
    ...
    Public Sub AbrirFormEnPanel(formToShow As Form)
        ...
        If Not Me.panelContenido.Controls.Contains(formToShow) Then
            ...
            If Not _formularios.ContainsValue(formToShow) Then
                AddHandler formToShow.FormClosed, Sub(s, args)
                                                      ...
                                                      ' Y volvemos al formulario de búsqueda.
                                                      btnBuscarFuncionario.PerformClick()
                                                      ActualizarBarraDeEstado() ' Actualiza cuando un temporal se cierra
                                                  End Sub
            End If
        End If
        ...
        ' Actualiza la barra de estado con la nueva información.
...
#Region "UI Helpers (Estilos, Foco y Barra de Estado)"

    ''' <summary>
    ''' Actualiza los labels de la barra de estado con la información actual de los formularios.
    ''' </summary>
    Private Sub ActualizarBarraDeEstado()
        ' Contamos todos los controles de tipo Form dentro del panel de contenido.
        Dim count As Integer = Me.panelContenido.Controls.OfType(Of Form)().Count()
        lblFormCount.Text = $"Formularios Abiertos: {count}"

        ' Mostramos el título del formulario actualmente activo.
        If _activeForm IsNot Nothing AndAlso Not _activeForm.IsDisposed Then
            lblActiveForm.Text = $"Activo: {_activeForm.Text} ({_activeForm.GetType().Name})"
        Else
            lblActiveForm.Text = "Activo: Ninguno"
        End If
    End Sub
    ...
    Private Sub frmDashboard_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Using uow As New UnitOfWork()
            TiposEstadoCatalog.Init(uow) ' ← UNA sola vez por proceso
        End Using
        ActualizarBarraDeEstado() ' <--- Llamada inicial
    End Sub
