...
#End Region
    ...
    ' <--- CORRECCIÓN: Se cambió el nombre del parámetro "ReadOnly" a "readOnlyMode"
    Public Sub New(novedadId As Integer, readOnlyMode As Boolean)
        Me.New(novedadId) ' Llama al constructor de edición para cargar los datos
        _readOnly = readOnlyMode
        _forzarPasos = False ' Permite navegar libremente por las pestañas en modo lectura
    End Sub
    ...
    Private Sub SetReadOnlyMode()
        ...
        btnAgregarFuncionario.Enabled = False
        btnQuitarFuncionario.Enabled = False
        btnAgregarFoto.Enabled = False
        btnEliminarFoto.Enabled = False
        btnGuardar.Visible = False

        If _pasoActual = PasoWizard.Confirmacion Then
            btnSiguiente.Text = "Cerrar"
        Else
            btnSiguiente.Text = "Siguiente >"
        End If
        RemoveHandler TabControlMain.Selecting, AddressOf TabControlMain_Selecting_Bloquear
    End Sub
    ...
    Private Sub CrearTabConfirmacion()
        ...
        lblConfTitulo = New Label() With {.Text = "Paso 4: Confirmá antes de guardar", .Font = New Font("Segoe UI", 12.0F, FontStyle.Bold), .Dock = DockStyle.Top, .Height = 32}
        Dim pnl As New TableLayoutPanel With {.Dock = DockStyle.Fill, .ColumnCount = 2, .RowCount = 4}
        ...
        pnl.RowStyles.Add(New RowStyle(SizeType.Percent, 25))
        pnl.RowStyles.Add(New RowStyle(SizeType.Percent, 30))
        lblConfFecha = New Label() With {.AutoSize = True, .Font = New Font("Segoe UI", 10.0F, FontStyle.Regular)}
        txtConfTexto = New TextBox() With {.Multiline = True, .ReadOnly = True, .ScrollBars = ScrollBars.Vertical, .Dock = DockStyle.Fill}
    ...
    Private Sub ActualizarResumenConfirmacion()
        ...
        For Each ctrl In flpFotos.Controls
            ...
            Dim thumb As New PictureBox With {.SizeMode = PictureBoxSizeMode.Zoom, .Size = New Size(96, 96), .BorderStyle = BorderStyle.FixedSingle, .Margin = New Padding(4)}
    ...
    Private Async Sub btnSiguiente_Click(sender As Object, e As EventArgs)
        If _readOnly AndAlso CType(sender, Button).Text = "Cerrar" Then
            Me.Close()
            Return
        End If
    ...
    Private Function ValidarPasoFuncionarios(Optional silent As Boolean = False) As Boolean
        If _funcionariosSeleccionados Is Nothing OrElse _funcionariosSeleccionados.Count = 0 Then
            If Not silent Then
...
#Region "Lógica de Fotos"
    ...
    Private Sub AgregarPictureBox(imageData As Byte(), tag As Object)
        Dim pic As New PictureBox With {.Image = CrearImagenCopia(imageData), .SizeMode = PictureBoxSizeMode.Zoom, .Size = New Size(120, 120), .Margin = New Padding(5), .BorderStyle = BorderStyle.FixedSingle, .Tag = tag, .Cursor = Cursors.Hand}
    ...
    Private Sub btnAgregarFoto_Click(sender As Object, e As EventArgs) Handles btnAgregarFoto.Click
        Using ofd As New OpenFileDialog With {.Filter = "Imágenes|*.jpg;*.jpeg;*.png;*.bmp", .Multiselect = True}
                ...
                For Each archivo In ofd.FileNames
                    Try
                        ...
                        If fi.Length > 15 * 1024 * 1024 Then
                            ...
                            Continue For
                        End If
                        If _rutasFotosNuevas.Contains(archivo) Then Continue For
