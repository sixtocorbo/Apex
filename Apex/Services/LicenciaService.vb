...
Public Class LicenciaService
    ...
    Optional tipoLicenciaId As Integer? = Nothing
        ...
        ' Usar IQueryable (lo devuelve el repositorio)
        Dim query As IQueryable(Of vw_LicenciasCompletas) =
            _unitOfWork.Repository(Of vw_LicenciasCompletas)().GetAll()
        ...
        Else
            ' Trae TODAS las licencias cuya categoría es de sanción
            ...
            Dim idsLicenciasSancion = Await _unitOfWork.Repository(Of TipoLicencia)().
                GetAll().
                Where(Function(tl) idsCategoriasSancion.Contains(tl.CategoriaAusenciaId)).
                Select(Function(tl) tl.Id).
                ToListAsync()

            query = query.Where(Function(s) idsLicenciasSancion.Contains(s.TipoLicenciaId))
        End If

        Dim lista = Await query.
            OrderByDescending(Function(s) s.FechaInicio).
            ToListAsync()
